...
use crate::encode::pprof::reusable::{StringID, StringTable};
...
impl PProfBuilder {
    ...
    pub fn add_ffi_sample(&mut self, frames: &[FFIFrame], values: &FFIHeapSampleValues) {
        println!(
            "ffi sample values {:?} {:?} {:?}",
            values.alloc_count as i64, values.alloc_space as i64, values.heap_space as i64
        );
        let mut sample = Sample {
        ...
        self.profile.sample.push(sample);
        println!("{:?} samples", self.profile.sample.len())
    }
